...
**Move to the next lesson** — Ctrl+Shift+P → *Run Task* → **Lessons: choose one**.
It lists every lesson with a one-line description, marks the one running now,
and switches the board to whichever number you pick. The lessons are already on
the board, so it takes effect immediately.

Or do it by hand: open `lessons\code.py` and change the one import:

```python
from L01_colors import main
```

Ctrl+S. If a lesson has a mistake in it, the traceback names the *lesson* file
and line, not `code.py`.
